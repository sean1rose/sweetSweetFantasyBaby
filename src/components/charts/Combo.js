  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="headerContainer">
          <h4>Consistency:</h4>
          <div><span>Number of {this.props.position === "wr" ? "WR1 Finishes: " : "RB1 Finishes: "}</span><span style={{"fontWeight": "bold"}}> {this.state.wrOneFinishes ? this.state.wrOneFinishes : this.state.rbOneFinishes}</span><span > out of 15 games ({this.state.rbOneFinishesPercent ? this.state.rbOneFinishesPercent : this.state.wrOneFinishesPercent}% of playable games)</span></div>    
          <div><span>Number of {this.props.position === "wr" ? "WR2 Finishes (24th ranked WR or above): " : "RB2 Finishes (24th ranked RB or above): "}</span><span style={{"fontWeight": "bold"}}> {this.state.wrTwoFinishes ? this.state.wrTwoFinishes : this.state.rbTwoFinishes }</span><span> out of 15 games ({this.state.rbTwoFinishesPercent ? this.state.rbTwoFinishesPercent : this.state.wrTwoFinishesPercent}% of playable games)</span></div>
          <div><span>Number of {this.props.position === "wr" ? "WR3 Finishes (36th ranked WR or above): " : "RB3 Finishes (36th ranked RB or above): "}</span><span style={{"fontWeight": "bold"}}> {this.state.wrThreeFinishes ? this.state.wrThreeFinishes : this.state.rbThreeFinishes}</span><span> out of 15 games ({this.state.rbThreeFinishesPercent ? this.state.rbThreeFinishesPercent : this.state.wrThreeFinishesPercent}% of playable games)</span></div>
        </div>
      </div>
    )
  }
